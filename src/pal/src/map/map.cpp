    ...
    )
{
    ...
    off_t adjust = MAPcalcAdj(offset);
    ...
    while (dirPos < dirSize)
    {
        ...
        if ((SIZE_T)(pageAddress - pWriteableRegion) >= cbWriteableRegion)
        {
            ...
            if (dwOldProtection != 0)
            {
                ...
                INT nProtect = W32toUnixAccessControl( dwOldProtection, FALSE );
                if (nProtect == 0)
                {
                    return false;
                }

                if (mprotect(pWriteableRegion, cbWriteableRegion, nProtect) != 0)
                {
                    return false;
                }
            ...
            IMAGE_SECTION_HEADER *pSection = NULL;
            {
                ...
                while (section < sectionEnd)
                {
                    ...
                    UINT alignUp = (value+alignment-1)&~(alignment-1);
                    if (rvaSect < (VAL32(section->VirtualAddress) + alignUp))
                    {
                        if (rvaSect < VAL32(section->VirtualAddress))
                        {
                            break;
                        }
                        else
            ...
            if (((pSection->Characteristics & VAL32(IMAGE_SCN_MEM_WRITE)) == 0))
            {
                ...
                INT nProtect = W32toUnixAccessControl( dwNewProtection, FALSE );
                if (nProtect == 0)
                {
                    return false;
                }
                ...
                if ((pSection->Characteristics & VAL32(IMAGE_SCN_MEM_WRITE)) != 0)
                {
                    ...
                    {
                        dwOldProtection = PAGE_READWRITE;
                    }
                }
                else
                {
                    if ((pSection->Characteristics & VAL32(IMAGE_SCN_MEM_EXECUTE)) != 0)
                    {
                        dwOldProtection = PAGE_EXECUTE_READ;
                    }
                    else
                    {
                        dwOldProtection = PAGE_READONLY;
                    }
                }
            }
        }
...
void * MAPMapPEFile(HANDLE hFile, LPCSTR szPath, SIZE_T *size, LPVOID lpPreloadedBase)
{
    ...
    {
        ERROR__(!doPreload,LOADER)( "Magic number mismatch\n" );
        palError = ERROR_INVALID_PARAMETER;
        goto done;
    }
    ...
    {
        ERROR__(!doPreload,LOADER)( "image is corrupt\n" );
        palError = ERROR_INVALID_PARAMETER;
        goto done;
    }
    ...
    for (unsigned i = 0; i < numSections; ++i)
    {
        ...
        {
            ERROR__(!doPreload,LOADER)( "section %d is corrupt\n", i );
            palError = ERROR_INVALID_PARAMETER;
            goto doneReleaseMappingCriticalSection;
        }
        if (currentHeader.Misc.VirtualSize > currentHeader.SizeOfRawData)
        {
            ERROR__(!doPreload,LOADER)( "no support for zero-padded sections, section %d\n", i );
            palError = ERROR_INVALID_PARAMETER;
            goto doneReleaseMappingCriticalSection;
        }
